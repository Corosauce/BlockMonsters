...
public class ProfilePhysics extends StructureProfileBase {

	public ProfilePhysics(LeafBlockMonster parJob) {
		super(parJob);
	}
	...
	public void populateStructureData() {
    	...
    	RigidBody rb = mainBody.initPhysicsObject(new BoxShape(new Vector3f(sizeX, sizeY, sizeZ)), new Vector3f((float)job.ent.posX, (float)job.ent.posY, (float)job.ent.posZ), 5F);
    	rb.setDamping(0, 0);
    	...
    	int limbOffsetX = (sizeX/2)+1;
    	int limbOffsetZ = (sizeZ/2)+1;
    	
    	StructurePiece limb = new StructurePiece(pieceIndex++, mainBody, limbOffsetX, 0, 0);
    	for (int length = 0; length < armLength; length++) {
    		limb.nodes.add(new StructureNode(limb, 0, -length, 0));
    		limb.nodes.add(new StructureNode(limb, 0, -length, -1));
    		limb.nodes.add(new StructureNode(limb, 0, -length, 1));
    	}
    	...
    	StructurePiece limb2 = new StructurePiece(pieceIndex++, mainBody, -limbOffsetX, 0, 0);
    	for (int length = 0; length < armLength; length++) {
    		limb2.nodes.add(new StructureNode(limb2, 0, -length, 0));
    		limb2.nodes.add(new StructureNode(limb2, 0, -length, -1));
    		limb2.nodes.add(new StructureNode(limb2, 0, -length, 1));
    	}
    	...
    	StructurePiece limb3 = new StructurePiece(pieceIndex++, mainBody, 0, 0, limbOffsetZ);
    	for (int length = 0; length < armLength; length++) {
    	...
    	StructurePiece limb4 = new StructurePiece(pieceIndex++, mainBody, 0, 0, -limbOffsetZ);
    	for (int length = 0; length < armLength; length++) {
    	...
    	if (FMLCommonHandler.instance().getEffectiveSide() == Side.SERVER) {
			...
			boolean useLinearReferenceFrameA = false;
			{
				...
				localA.origin.set(-limbOffsetX, 0.0F, 0F);
				...
				float range = limiterSide;
				float rangeMain = limiterMovement;
				float offsetAdj = (float) Math.toRadians(-legOffset2);
				
				/*tmp.set(-BulletGlobals.SIMD_PI * range, -BulletGlobals.SIMD_PI * range, offsetAdj+(-BulletGlobals.SIMD_PI * rangeMain));
				...
				/*tmp.set(0, -BulletGlobals.SIMD_PI * wat, -BulletGlobals.SIMD_PI * moveRange);
				joint6DOF.setAngularLowerLimit(tmp);
				tmp.set(0, BulletGlobals.SIMD_PI * wat, BulletGlobals.SIMD_PI * moveRange);
				joint6DOF.setAngularUpperLimit(tmp);*/
				
				tmp.set(0, 0, -BulletGlobals.SIMD_PI);
				joint6DOF.setAngularLowerLimit(tmp);
				tmp.set(0, 0, -BulletGlobals.SIMD_PI);
				joint6DOF.setAngularUpperLimit(tmp);
			...
			
			{
				...
				localB.origin.set(0f, 0F, 0f);
				localA.origin.set(limbOffsetX, 0, 0F);
				...
				/*float range = limiterSide;
				float rangeMain = limiterMovement;
				float offsetAdj = (float) Math.toRadians(-legOffset2-legOffset3);
			...
			
			{
				...
				localB.origin.set(0f, 0F, 0f);
				localA.origin.set(0F, 0, -limbOffsetZ);
				...
				/*tmp.set((-BulletGlobals.SIMD_PI * range), offsetAdj+(-BulletGlobals.SIMD_PI * rangeMain), (-BulletGlobals.SIMD_PI * range));
				joint6DOF.setAngularLowerLimit(tmp);
				tmp.set((BulletGlobals.SIMD_PI * range), offsetAdj+(BulletGlobals.SIMD_PI * rangeMain), (BulletGlobals.SIMD_PI * range));
				joint6DOF.setAngularUpperLimit(tmp);*/
				
				/*tmp.set(offsetAdj+(-BulletGlobals.SIMD_PI * rangeMain), -BulletGlobals.SIMD_PI * range, (-BulletGlobals.SIMD_PI * range));
				joint6DOF.setAngularLowerLimit(tmp);
				tmp.set(offsetAdj+(BulletGlobals.SIMD_PI * rangeMain), BulletGlobals.SIMD_PI * range, (BulletGlobals.SIMD_PI * range));
				joint6DOF.setAngularUpperLimit(tmp);*/
				
				tmp.set(-BulletGlobals.SIMD_PI, 0, 0);
				joint6DOF.setAngularLowerLimit(tmp);
				tmp.set(-BulletGlobals.SIMD_PI, 0, 0);
				joint6DOF.setAngularUpperLimit(tmp);
			...
			
			{
				...
				/*float range = limiterSide;
				float rangeMain = limiterMovement;
				float offsetAdj = (float) Math.toRadians(legOffset3+legOffset2);
    	...
    	limb = new StructurePiece(pieceIndex++, mainBody, 4, 0, 0);
    	for (int i = 0; i < armLength; i++) limb.nodes.add(new StructureNode(limb, i, 0, 0));
    	limb.initPhysicsObject();
    	//mainBody.addChildPiece(limb);
    	
    	StructurePiece armRight2 = new StructurePiece(pieceIndex++, limb, armLength, 0, 0);
    	for (int i = 0; i < armLengthB; i++) armRight2.nodes.add(new StructureNode(armRight2, i, 0, 0));
    	armRight2.initPhysicsObject();
    	limb.addChildPiece(armRight2);
    	
    	StructurePiece armRight3 = new StructurePiece(pieceIndex++, armRight2, armLength, 0, 0);
    	for (int i = 0; i < armLengthB; i++) armRight3.nodes.add(new StructureNode(armRight3, i, 0, 0));
    	armRight3.initPhysicsObject();
    	armRight2.addChildPiece(armRight3);
    	
    	StructurePiece armRight4 = new StructurePiece(pieceIndex++, armRight3, armLength, 0, 0);
    	for (int i = 0; i < armLengthB; i++) {
    		if (i < armLengthB-1) {
    			StructureNode sn = new StructureNode(armRight4, i, 0, 0);
    			collisionListeners.add(sn);
    			armRight4.nodes.add(sn);
    		} else {
    			armRight4.nodes.add(new StructureNode(armRight4, i, 0, 0));
    		}
    	}
    	armRight4.initPhysicsObject();
    	armRight3.addChildPiece(armRight4);
    	...
    	armRight2 = new StructurePiece(pieceIndex++, limb, -armLength, 0, 0);
    	for (int i = 0; i < armLengthB; i++) armRight2.nodes.add(new StructureNode(armRight2, -i, 0, 0));
    	armRight2.initPhysicsObject();
    	limb.addChildPiece(armRight2);
    	
    	armRight3 = new StructurePiece(pieceIndex++, armRight2, -armLength, 0, 0);
    	for (int i = 0; i < armLengthB; i++) armRight3.nodes.add(new StructureNode(armRight3, -i, 0, 0));
    	armRight3.initPhysicsObject();
    	armRight2.addChildPiece(armRight3);
    	
    	armRight4 = new StructurePiece(pieceIndex++, armRight3, -armLength, 0, 0);
    	for (int i = 0; i < armLengthB; i++) {
    		if (i < armLengthB-1) {
    			StructureNode sn = new StructureNode(armRight4, -i, 0, 0);
    			collisionListeners.add(sn);
	...
	public void tickUpdate() {
		...
		try {
			...
			for (int i = 0; i < 4; i++) {
				...
				//body.childPieces.get(i).applyThrustForce(new Vector3f(0, -100, 0), new Vector3f(1, 0, 0));
			}
			...
			float offset = 45;
			float amp = -10+((float)Math.sin(job.ent.worldObj.getTotalWorldTime() * 0.1F) * 20F);
			float amp2 = -10+((float)Math.cos(40+job.ent.worldObj.getTotalWorldTime() * 0.1F) * 20F);
			
			float amp3 = (float)Math.sin(job.ent.worldObj.getTotalWorldTime() * 0.05F) * 2F;
			float amp4 = (float)Math.sin(job.ent.worldObj.getTotalWorldTime() * 0.05F) * 2F;
			...
			if (!hasAnimationSequenceRunning("pivotThingB")) {
				...
				//seqA.add(new PhysicsMuscle(job.ent.worldObj, seqA, body.childPieces.get(0), Vec3.createVectorHelper(0, 0, 1), 90));
				//seqA.add(new PhysicsMuscle(job.ent.worldObj, seqA, body.childPieces.get(1), Vec3.createVectorHelper(1, 0, 0), -amp));
			...
			if (!hasAnimationSequenceRunning("pivotThingC")) {
				...
				seqA.add(new PhysicsMuscle(job.ent.worldObj, seqA, body.childPieces.get(2), Vec3.createVectorHelper(1, 0, 0), -offset-amp));
			...
			if (!hasAnimationSequenceRunning("pivotThingD")) {
				...
				seqA.add(new PhysicsMuscle(job.ent.worldObj, seqA, body.childPieces.get(3), Vec3.createVectorHelper(1, 0, 0), offset+amp));
			...
			//if (job.ent.worldObj.getTotalWorldTime() % 100 == 0) {
				...
				job.ent.setPosition(trns.origin.x, trns.origin.y, trns.origin.z);
